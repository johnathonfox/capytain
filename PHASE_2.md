...
- **Sync engine + outbox + drain:** `capytain-sync` owns the per-folder + per-account sync loop, the outbox drain (5s tick, exponential backoff, DLQ on `MAX_ATTEMPTS = 5`), and the reactive event channel. The outbox is op-kind-keyed JSON payloads, pluggable for new mutations.
...
- **Auth:** OAuth2 + PKCE for Gmail and Fastmail via `capytain-auth`; access tokens minted on demand and cached in the desktop's backend factory. SMTP XOAUTH2 reuses the same token mint.
